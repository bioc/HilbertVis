...
overall statistical structure and the distribution of features, special
...
The command \texttt{make RandomTestData} (which is provided only for demonstration purposes) 
...
to fulfil the requirements just outlined as well as possible. To my knowledge, 
...
To plot the matrix, just pass it to the standard function \texttt{image}. By
default, \texttt{image} uses a colour palette ranging from red to yellow, which
is not too suitable for this kind of data. I prefer the low values to be white, so that the 
peaks with high values come out with more contrast. Hence, the function 
\texttt{hilbertDefaultPalette} produces a colour palette that ranges from white 
over blue to red and is well suited for typical Hilbert plots. The following
command (used to produce Fig.\ \ref{HilbertTest}) passes this palette to \texttt{image}'s 
\texttt{col} argument and also suppresses plotting of the axes as they are not
informative.
<<HilbertTest,fig=TRUE,include=FALSE>>=
image( hMat, 
   col=hilbertDefaultPalette( max(hMat), asArray=FALSE ),
   xaxt="n", yaxt="n" )
@

In the present case, the range of values in \texttt{hMat} is only in the hundreds
<<>>=
range( hMat )
@
If you have larger ranges, you may want to rescale \texttt{hMat} before passing it
to \texttt{image} in order to be able to use a palette of reasonable size.
...
the \texttt{EBImage} package that allows to suppress scaling. As \texttt{display}
cannot take an ordinary matrix but requires to be passed a special \texttt{Image}
object, \texttt{hilbertImage} can be asked to produce such an object:
<<eval=FALSE>>=
img <- hilbertImage( vec, forEBImage=TRUE )
display( img )
@
Note that the image is automatically coloured with the 
\texttt{hilbertDefaultPalette}.
...
This is a simple wrapper around the function \texttt{plotLongVector} discussed earlier. 
Here is the definition of simpleLinPlot:
<<>>=
simpleLinPlot
@
...
\textit{Note:} \texttt{biocLite} does not work yet as \texttt{HilbertVis} is still in the
Bioconductor peer-review process. Until then, you need to download the package 
from the SVN system.
...
\textbf{Linux and MacOS:} To use \texttt{HilbertVisGUI}, you hence need to 
have installed (besides a reasonably 
recent version of R, at least R 2.7.0) the 'gtkmm' library for GTK+ 2.4, with its 
header files.
...
If you do not get any output, \texttt{gtkmm-2.4} is missing or cannot be found by 
\texttt{pkg-config}.

In most Linux systems, you only need to use your distribution's package manager
and install the \texttt{gtkmm} development package. See the following URL to learn how
the package is called on the different distributions: \url{http://www.gtkmm.org/download.shtml}
   
Note that at the above URL, there is also a Fink package for use with MacOS X.

Once you have verified that \texttt{gtkmm} is available, install \texttt{HilbertVisGUI} as 
usual with \texttt{biocLite}.
\begin{verbatim}
source( "http://bioconductor.org/biocLite.R" )
...
\textbf{MS Windows:} For Windows, you can find binaries of \texttt{gtkmm} on the Gnome FTP server. As of
now, the URL is \url{http://ftp.gnome.org/pub/gnome/binaries/win32/gtkmm/}

Note that you need the `gtkmm-win32-devel' package only if you want to install 
HilbertVisGUI from source. If you have a pre-compiled binary
package, the 'gtkmm-win32-runtime' package is sufficient.

If you compile from source, make sure that the environment variable \texttt{GTK\_PATH} is set and
gives the path to the GTK+/gtkmm installation.

Again, once \texttt{gtkmm} is available, install \texttt{HilbertVisGUI} as 
usual with \texttt{biocLite}.
\begin{verbatim}
source( "http://bioconductor.org/biocLite.R" )
biocLite( "HilbertVisGUI" )
